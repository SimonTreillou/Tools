...
if __name__ == "__main__":
    ...
    for i, name in enumerate(variable_params['JobName']):
        ...
        # Bathymetry and grid
        #
        if "GridOption" in fixed_params:
            GridOption=fixed_params["GridOption"]
        else:
            GridOption="3DPlanar"
            print("No grid option provided. Default: "+str(base_params["GridOption"]))
        ...
        Lx,Ly,Mx,My,dx,dy=make_grid(".","CROCO",args)
        write_grid("./Prod/"+name,Lx,Ly,Mx-2,My-2,Mz,"grid.nc","CROCO")
        shutil.copyfile("./grid.nc", "./Prod/"+name+"/grid.nc")
        ...
        write_bdrag("./Prod/"+name,bdrag,"CROCO")
        ...
        if CPUs==None:
            CPUs=extract_mpi("./Routines","CROCO")
        else:
        ...
        os.chdir("./Prod/"+name)
        create_jobsub(name,JobTime,CPUs,config_name,"CROCO")
        os.chdir("../..")
        print('**************************\n')